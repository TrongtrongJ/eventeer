...
describe('CouponsService', () => {
    ...
    describe('validateAndApply', () => {
        it('should apply percentage discount correctly', async () => {
            ...
            const result = await service.validateAndApply({ code: 'SAVE20', eventId: 'event-123' }, 100, 'test-correlation-id');
        ...
        it('should apply fixed discount correctly', async () => {
            ...
            mockRedis.incr.mockResolvedValue(51);
            mockRepository.save.mockResolvedValue({ ...mockCoupon, currentUsages: 51 });
            const result = await service.validateAndApply({ code: 'FIXED10', eventId: 'event-123' }, 100, 'test-correlation-id');
            expect(result.isValid).toBe(true);
        ...
        it('should reject expired coupon', async () => {
            ...
            mockRedis.set.mockResolvedValue('OK');
            mockRepository.findOne.mockResolvedValue(mockCoupon);
            const result = await service.validateAndApply({ code: 'EXPIRED', eventId: 'event-123' }, 100, 'test-correlation-id');
        ...
        it('should reject coupon when usage limit reached', async () => {
            ...
            const result = await service.validateAndApply({ code: 'MAXED', eventId: 'event-123' }, 100, 'test-correlation-id');
            expect(result.isValid).toBe(false);
        });
    });
});
