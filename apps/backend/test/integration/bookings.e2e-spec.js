    ...
    describe('1. User Registration & Authentication', () => {
        ...
        it('should register an organizer user', async () => {
            const response = await request(app.getHttpServer())
                ...
                .expect(201);
            await dataSource.query(`UPDATE users SET role = 'ORGANIZER' WHERE email = 'organizer@test.com'`);
    ...
    describe('4. Booking Creation (Authenticated)', () => {
        ...
        it('should update event available seats after booking', async () => {
            const response = await request(app.getHttpServer())
                .get(`/events/${eventId}`)
                .expect(200);
            expect(response.body.data.availableSeats).toBe(98);
        });
    ...
    describe('8. Booking Cancellation', () => {
        ...
        it('should cancel booking successfully', async () => {
            ...
            const seatsAfter = await request(app.getHttpServer())
                .get(`/events/${eventId}`)
                .expect(200);
            expect(seatsAfter.body.data.availableSeats).toBe(seatsBefore.body.data.availableSeats + 1);
        });
